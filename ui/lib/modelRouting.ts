...
export function modelRuleScopeRank(
  ...
  if (projectId && workflowId && rule.project_id === projectId && rule.workflow_id === workflowId) return 0;
  if (projectId && rule.project_id === projectId && rule.workflow_id == null) return 1;
  if (rule.project_id == null && rule.workflow_id == null) return 2;
  return 3;
}
...
export function resolveEffectiveModel(
  ...
  const bestRank = Math.min(...enabledRules.map(rule => modelRuleScopeRank(rule, projectId, workflowId)));
  const sorted = enabledRules
    .filter(rule => modelRuleScopeRank(rule, projectId, workflowId) === bestRank)
