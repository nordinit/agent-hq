    ...
    {mode === 'advanced' ? <Field label="Versioned definition" hint="Uses the same server validation and evaluator as the guided builder. Preview before saving."><textarea spellCheck={false} className={`${inputClass} min-h-[440px] font-mono text-xs`} value={draftText} onChange={event => { setModeError(null); onChange(event.target.value); }}/></Field> : <>
      ...
      <Section title="Population" description="Choose which records qualify within the project, workflow, and task scope above. Title matching and this filter both apply.">
        ...
        {(populationOpen || guide.filterField) ? <div className="space-y-3">
          <div className="flex items-start gap-2"><div className="min-w-0 flex-1"><TelemetryChoicePicker label="Population field" value={guide.filterField} onChange={selectFilter} options={telemetryFieldChoices(populationFields, currentCatalog)} scopeCatalog={currentCatalog} placeholder="Choose a field to filter"/></div><Button type="button" size="sm" variant="ghost" className="mt-6" aria-label="Remove population filter" onClick={() => { update({ filterField: '', filterValue: '' }); setPopulationOpen(false); }}><Trash2 className="h-4 w-4"/></Button></div>
      ...
      <Section title="Attribution" description="Choose which agent receives credit for the measured work.">
        ...
        {guide.attribution === 'assigned_agent_at_entry' && requirements.journey && <TelemetryChoicePicker label="Journey entry (required for attribution)" value={guide.start} onChange={value => update({ start: value })} options={signals} scopeCatalog={currentCatalog} hint="Use the assignment recorded at this moment. Missing historical assignments remain Unknown."/>}
      ...
      {guide.recipe !== 'count' && <Section title="Measurement details" description={recipes.find(recipe => recipe.value === guide.recipe)?.help}>
        {guide.recipe === 'numeric' && <div className="grid gap-3 sm:grid-cols-2"><TelemetryChoicePicker label="Canonical numeric field" value={guide.field} onChange={value => update({ field: value, unit: numericFields.find(field => field.id === value)?.unit ?? '' })} options={telemetryFieldChoices(numericFields, currentCatalog)} scopeCatalog={currentCatalog} placeholder="Choose a numeric field"/><Select label="Value at" value={guide.basis} onChange={value => update({ basis: value as TelemetryGuide['basis'] })} options={timings} hint="Historical values require recorded snapshots; missing values are disclosed."/></div>}
        ...
        {requirements.success && <div className="grid gap-3 sm:grid-cols-2">{requirements.start && guide.attribution !== 'assigned_agent_at_entry' && <TelemetryChoicePicker label="Journey / interval starts when" value={guide.start} onChange={value => update({ start: value })} options={signals} scopeCatalog={currentCatalog}/>}<TelemetryChoicePicker label={guide.recipe === 'milestone' || (guide.recipe === 'numeric' && guide.basis === 'at_event') ? 'Measured milestone' : 'Success / interval ends when'} value={guide.success} onChange={value => update({ success: value })} options={signals} scopeCatalog={currentCatalog}/></div>}
