...
import { X, Pencil, AlertTriangle, ChevronDown, ExternalLink, StopCircle, Trash2, Activity, Cpu, Layers, PauseCircle, PlayCircle, Plus, Search, GitBranch, Maximize2, ArrowLeft } from 'lucide-react';
...
const PAGE_CARD = 'rounded-xl border border-slate-800 bg-slate-900/60 p-5';
...
interface Props {
  ...
  /** 'panel' is the slide-over on the board; 'page' is the full-page view at /tasks/[id]. */
  variant?: 'panel' | 'page';
}
...
function RelatedTasksSection({
  ...
  /** Called before following a link to another task, so a panel can close itself. */
  onNavigateAway?: () => void;
  ...
  const openRelatedTask = useCallback((relatedTaskId: number) => {
    onNavigateAway?.();
    router.push(taskHref(relatedTaskId));
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <Link
                            href={taskHref(relatedTask.id)}
                            onClick={(event) => { event.stopPropagation(); onNavigateAway?.(); }}
...
export function TaskDetailPanel({ task, statuses, onClose, onSave, onDelete, onCancel, onPause, onUnpause, variant = 'panel' }: Props) {
  ...
  // A live refresh can push one mid-edit too; that must not discard the operator's changes.
  const modeRef = useRef(mode);
  modeRef.current = mode;
  useEffect(() => {
    setLocalTask(task);
    if (modeRef.current !== 'edit') setForm({ ...task, recurring: !!task.recurring });
  }, [task]);
  ...
  // Close on Escape. The full page is a route, not an overlay, so there is nothing to close.
  useEffect(() => {
    if (isPage) return;
  ...
  // Links out of the panel close it first; the full page has nothing to close.
  const dismiss = isPage ? undefined : onClose;
  const taskHref = (id: number) => (isPage ? `/tasks/${id}` : `/tasks?id=${id}`);

  const actionButtons = mode === 'view' && (
    <>
      {localTask.active_instance_id && onCancel && !['done', 'cancelled', 'failed'].includes(localTask.status) && (
        <button
      ...
      {!localTask.paused_at && onPause && !['done', 'cancelled', 'failed'].includes(localTask.status) && (
        <button
          onClick={handlePause}
          disabled={pausing}
  ...
  const titleBlock = (
    <div className="flex items-start gap-2">
      ...
      <h3 className={`${isPage ? 'text-2xl' : 'text-lg'} font-bold text-white leading-snug`}>
        {localTask.title}
        {localTask.recurring ? (
  ...
  const pausedBanner = localTask.paused_at && (
  ...
  const observabilityBlock = (localTask.active_instance_id || localTask.latest_artifact_summary || localTask.last_agent_heartbeat_at || localTask.run_is_stale) && (
    <div>
  ...
  const editFooter = mode === 'edit' && (
    <div className={`flex flex-col gap-2 px-6 py-4 border-t border-slate-700 shrink-0${isPage ? ' mt-6' : ''}`}>
      {saveError && (
  ...
  if (isPage) {
    return (
      ...
      {deleteConfirm && (
        ...
        <div className="flex flex-1 min-h-0 flex-col overflow-y-auto bg-slate-950">
          <div className="sticky top-0 z-20 border-b border-slate-800 bg-slate-950/95 backdrop-blur">
            <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 md:px-6">
              <div className="flex min-w-0 items-center gap-2">
                <Link
                  href="/tasks"
                  className="inline-flex shrink-0 items-center gap-1 rounded-md px-1.5 py-1 text-xs text-slate-400 transition-colors hover:bg-slate-800 hover:text-white"
                >
                  <ArrowLeft className="h-3.5 w-3.5" />
                  Tasks
                </Link>
                <span className="text-slate-700">/</span>
                <span className="shrink-0 font-mono text-sm text-slate-500">#{localTask.id}</span>
          ...
          <div className="mx-auto w-full max-w-7xl px-4 py-6 pb-24 md:px-6 md:pb-10">
            {mode === 'view' ? (
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
                <div className="min-w-0 space-y-6">
                  ...
                  <TaskFieldsSection
                    ...
                    task={localTask}
                  />
                  <section className={PAGE_CARD}>{runsBlock}</section>
                  <section className={PAGE_CARD}>{relatedBlock}</section>
                  <section className={PAGE_CARD}><NotesSection taskId={localTask.id} /></section>
                  <section className={PAGE_CARD}>{historyBlock}</section>
                </div>
                ...
                <aside className="min-w-0 space-y-6">
                  ...
                  {observabilityBlock}
                  <section className={PAGE_CARD}><AttachmentsSection taskId={localTask.id} /></section>
                </aside>
              </div>
            ) : (
