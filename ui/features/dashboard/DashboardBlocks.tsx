...
export function DashboardDetails({ selected, data, page, catalog, close, resource }: { selected: DashboardInspection | null; data: Record<string, DashboardMetricState>; page: DashboardDocument; catalog: TelemetryCatalog | null; close: () => void; resource?: { dashboardId?: string; reportId?: string } }) {
  ...
  return <dialog ref={ref} className={`${styles.dialog} ${styles.drawer}`} onClose={close} aria-labelledby="dashboard-detail-title"><div className={styles.drawerHeader}><h2 id="dashboard-detail-title">{selected?.title}</h2><button autoFocus type="button" className={styles.button} onClick={close} aria-label="Close details"><X/></button></div>
    {selected?.operation ? <><p className={styles.description}>Operational statistics use the existing project-scoped run and task records. Run totals cover runs created in the last 24 hours; completed tasks use their recorded completion time.</p><div className={styles.filters}><Link className={styles.button} href={selected.operation === 'agents' ? '/agents' : selected.operation === 'templates' ? '/tasks/recurring' : '/settings/logs'}>Open {selected.operation === 'agents' ? 'agents' : selected.operation === 'templates' ? 'recurring tasks' : 'execution logs'} <ArrowUpRight/></Link></div></> : state?.result ? <>
      <div className={styles.filters}><span>Exact value: {telemetryExactValue(state.result.value)}</span>{binding && resource && <Link className={styles.button} href={`/telemetry?tab=analyze&${resource.dashboardId ? `dashboard_id=${encodeURIComponent(resource.dashboardId)}` : `report_id=${encodeURIComponent(resource.reportId!)}`}&widget_id=${encodeURIComponent(binding.id)}${page.scope?.project_id ? `&project_id=${page.scope.project_id}` : ''}`}>Open in Analyze <ArrowUpRight/></Link>}</div>
