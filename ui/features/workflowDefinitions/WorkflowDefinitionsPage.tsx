...
import { api, type WorkflowTypeConfig, type WorkflowTypeOutcome, type TaskFieldSchema, type TaskRelationshipTypeConfig, type TaskStatusMeta } from '@/lib/api';
...
import {
  ...
  type WorkflowDefinitionTab,
  type WorkflowTypeForm,
} from './workflowDefinitionShared';

export default function WorkflowDefinitionsPage() {
  const [config, setConfig] = useState<WorkflowTypeConfig[]>([]);
  const [selectedKey, setSelectedKey] = useState<string>('');
  const [loading, setLoading] = useState(true);
  const [notice, setNotice] = useState<Notice>(null);
  const [activeTab, setActiveTab] = useState<WorkflowDefinitionTab>('overview');
  const [creatingWorkflowType, setCreatingWorkflowType] = useState(false);
  const [workflowTypeForm, setWorkflowTypeForm] = useState<WorkflowTypeForm>(emptyWorkflowTypeForm);
  ...
  const load = async (preferredKey?: string) => {
    ...
    try {
      ...
      const workflowTypes = response.workflow_types ?? [];
      setConfig(workflowTypes);
      const nextKey = preferredKey && workflowTypes.some(type => type.key === preferredKey)
        ? preferredKey
        : workflowTypes[0]?.key ?? '';
  ...
  const workflowTypeDeletionState = useMemo(() => {
    if (!selectedWorkflowType) return null;
    return selectedWorkflowType.deletion ?? {
      protected: selectedWorkflowType.key === 'generic',
      reason: selectedWorkflowType.key === 'generic' ? 'generic' : null,
  ...
  useEffect(() => {
    ...
    setWorkflowStatuses((selectedWorkflowType.statuses ?? []).slice().sort((a, b) => (a.stage_order ?? 0) - (b.stage_order ?? 0)));
    setWorkflowTypeForm({
      key: selectedWorkflowType.key,
      name: selectedWorkflowType.name,
      description: selectedWorkflowType.description,
    });
    setTaskTypesText(selectedWorkflowType.task_types.map(taskType => taskType.task_type).join('\n'));
    setCreatingWorkflowType(false);
  ...
  const reloadStatuses = async (workflowTypeKey: string) => {
    setStatusLoading(true);
    try {
      const response = await api.getWorkflowTypeStatuses(workflowTypeKey);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setStatusLoading(true);
    api.getWorkflowTypeStatuses(selectedWorkflowType.key)
      .then(response => {
        if (!cancelled) {
          setWorkflowStatuses((response.statuses ?? []).slice().sort((a, b) => (a.stage_order ?? 0) - (b.stage_order ?? 0)));
        }
  ...
  const baseFieldSchema = useMemo(
    () => selectedWorkflowType?.field_schemas.find(schema => schema.task_type == null) ?? null,
    [selectedWorkflowType],
  );
  const baseOutcomes = useMemo(
    () => (selectedWorkflowType?.outcomes ?? []).filter(outcome => outcome.task_type == null && !BACKEND_ONLY_OUTCOMES.has(outcome.outcome_key)),
    [selectedWorkflowType],
  );
  ...
  const overrideFieldSchemas = useMemo(
    () => (selectedWorkflowType?.field_schemas ?? [])
  ...
  const relationshipTypesForTable = useMemo(
    () => selectedWorkflowType?.relationship_types ?? [],
    [selectedWorkflowType],
  );
  ...
  const editOutcome = (outcome: WorkflowTypeOutcome) => {
  ...
  const submitCreateWorkflowType = async () => {
    if (!workflowTypeForm.key.trim() || !workflowTypeForm.name.trim()) {
      setNotice({ type: 'error', message: 'Workflow type key and name are required.' });
      return;
    }
    setSaving('create-workflow-type');
    try {
      await api.createWorkflowType({
        key: workflowTypeForm.key.trim(),
        name: workflowTypeForm.name.trim(),
        description: workflowTypeForm.description.trim(),
      });
      await load(workflowTypeForm.key.trim());
      setCreatingWorkflowType(false);
      setSuccess(`Created workflow type ${workflowTypeForm.key.trim()}.`);
    } catch (error) {
      setError(error);
    } finally {
      setSaving(null);
  ...
  const saveWorkflowType = async () => {
    if (!selectedWorkflowType) return;
    setSaving('workflow-type');
    try {
      await api.updateWorkflowType(selectedWorkflowType.key, {
        name: workflowTypeForm.name.trim(),
        description: workflowTypeForm.description.trim(),
      });
      await load(selectedWorkflowType.key);
      setSuccess(`Saved workflow type ${selectedWorkflowType.key}.`);
    } catch (error) {
      setError(error);
    } finally {
      setSaving(null);
  ...
  const deleteWorkflowType = async () => {
    if (!selectedWorkflowType) return;
    if (workflowTypeDeletionState?.protected) {
      const message = workflowTypeDeletionState.reason === 'generic'
        ? 'The generic workflow type is protected and cannot be deleted.'
        : workflowTypeDeletionState.reason === 'open_workflows'
          ? `This workflow type cannot be deleted while ${workflowTypeDeletionState.open_workflow_count} open workflow${workflowTypeDeletionState.open_workflow_count === 1 ? '' : 's'} reference it.`
          : 'This workflow type cannot be deleted right now.';
      window.alert(message);
      setNotice({ type: 'error', message });
      return;
    }
    if (!window.confirm(`Delete workflow type ${selectedWorkflowType.key}?`)) return;
    setSaving('delete-workflow-type');
    try {
      await api.deleteWorkflowType(selectedWorkflowType.key);
      await load();
      setSuccess(`Deleted workflow type ${selectedWorkflowType.key}.`);
    } catch (error) {
      setError(error);
    } finally {
      setSaving(null);
  ...
  const saveTaskTypes = async () => {
    ...
    try {
      await api.replaceWorkflowTypeTaskTypes(selectedWorkflowType.key, taskTypes);
      await load(selectedWorkflowType.key);
      setSuccess(`Updated allowed task types for ${selectedWorkflowType.key}.`);
    } catch (error) {
      setError(error);
    } finally {
      setSaving(null);
  ...
  const saveSchema = async () => {
    if (!selectedWorkflowType || !schemaEditor) return;
    ...
    try {
      ...
      if (schemaEditor.id) {
        await api.updateTaskFieldSchema(selectedWorkflowType.key, schemaEditor.id, payload);
      } else {
        await api.createTaskFieldSchema(selectedWorkflowType.key, payload);
      }
      await load(selectedWorkflowType.key);
      closeSchemaEditor();
  ...
  const saveOutcome = async () => {
    if (!selectedWorkflowType || !outcomeEditor) return;
    setSaving('outcome');
    try {
      const payload = {
      ...
      if (outcomeEditor.id) {
        await api.updateWorkflowOutcome(selectedWorkflowType.key, outcomeEditor.id, payload);
      } else {
        await api.createWorkflowOutcome(selectedWorkflowType.key, payload);
      }
      await load(selectedWorkflowType.key);
      closeOutcomeEditor();
  ...
  const saveRelationshipType = async () => {
    if (!selectedWorkflowType || !relationshipTypeEditor) return;
    ...
    try {
      ...
      if (relationshipTypeEditor.id) {
        await api.updateWorkflowRelationshipType(selectedWorkflowType.key, relationshipTypeEditor.id, payload);
      } else {
        await api.createWorkflowRelationshipType(selectedWorkflowType.key, payload);
      }
      await load(selectedWorkflowType.key);
  ...
  const deleteRelationshipType = async (relationshipType: TaskRelationshipTypeConfig) => {
    if (!selectedWorkflowType || !window.confirm(`Delete relationship type ${relationshipType.key}?`)) return;
    setSaving(`delete-relationship-type-${relationshipType.id}`);
    try {
      await api.deleteWorkflowRelationshipType(selectedWorkflowType.key, relationshipType.id);
      await load(selectedWorkflowType.key);
  ...
  const deleteOutcome = async (outcome: WorkflowTypeOutcome) => {
    if (!selectedWorkflowType || !window.confirm('Delete this outcome definition?')) return;
    setSaving(`delete-outcome-${outcome.id}`);
    try {
      await api.deleteWorkflowOutcome(selectedWorkflowType.key, outcome.id);
      await load(selectedWorkflowType.key);
  ...
  const deleteSchema = async () => {
    if (!selectedWorkflowType || !schemaDeleteDialog) return;
    ...
    try {
      await api.deleteTaskFieldSchema(selectedWorkflowType.key, schema.id);
      await load(selectedWorkflowType.key);
  ...
  const tabs: Array<{ id: WorkflowDefinitionTab; label: string; count?: number }> = [
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className={SCOPE_CARD_CLASS} data-tour-target="workflow-definitions-main">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-amber-300">Workflow Type</p>
            <p className="mt-1 text-base font-semibold text-white">
              {selectedWorkflowType ? `${selectedWorkflowType.name} (${selectedWorkflowType.key})` : 'Select or create a workflow type.'}
            </p>
          ...
          <div className="grid gap-3 sm:grid-cols-[minmax(280px,1fr)_auto] lg:min-w-[620px]">
            ...
            <Button
              size="sm"
              variant="secondary"
              onClick={() => { setCreatingWorkflowType(true); setWorkflowTypeForm(emptyWorkflowTypeForm); }}
            >
        ...
        {creatingWorkflowType && (
          <div className="mt-4 grid gap-3 rounded-xl border border-slate-700 bg-slate-900/70 p-3 lg:grid-cols-[180px_220px_minmax(0,1fr)_auto]">
            <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="key" value={workflowTypeForm.key} onChange={e => setWorkflowTypeForm(form => ({ ...form, key: e.target.value }))} />
            <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="name" value={workflowTypeForm.name} onChange={e => setWorkflowTypeForm(form => ({ ...form, name: e.target.value }))} />
            <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="description" value={workflowTypeForm.description} onChange={e => setWorkflowTypeForm(form => ({ ...form, description: e.target.value }))} />
            <div className="flex gap-2">
              <Button size="sm" variant="primary" loading={saving === 'create-workflow-type'} onClick={submitCreateWorkflowType}>Create</Button>
              <Button size="sm" variant="ghost" onClick={() => setCreatingWorkflowType(false)}>Cancel</Button>
            </div>
          </div>
        )}
      </Card>
          ...
          <div className="relative w-full max-w-md space-y-5 rounded-xl border border-slate-700 bg-slate-800 p-6 shadow-xl">
            ...
            <div className="rounded-lg border border-red-500/40 bg-red-950/30 p-4">
              <div className="flex items-start gap-2 text-sm text-red-100/90">
                ...
                <p>
                  This will permanently delete the <span className="font-mono text-red-100">{schemaDeleteDialog.label}</span> schema from workflow type <span className="font-mono text-red-100">{selectedWorkflowType?.key}</span>. This removes shared configuration, not just a local UI preference.
                </p>
              </div>
            </div>
          ...
          {activeTab === 'overview' && (
            <Card className="space-y-6">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <Button size="sm" variant="danger" loading={saving === 'delete-workflow-type'} onClick={deleteWorkflowType}>
                ...
                <div className="space-y-4">
                  <div className="grid gap-4 md:grid-cols-[minmax(160px,0.45fr)_minmax(0,1fr)]">
                    <label className="block">
                      ...
                      <input className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-400" value={workflowTypeForm.key} disabled />
                    </label>
                    <label className="block">
                      <span className="mb-1 block text-xs uppercase tracking-wide text-slate-500">Name</span>
                      <input className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" value={workflowTypeForm.name} onChange={e => setWorkflowTypeForm(form => ({ ...form, name: e.target.value }))} />
                    </label>
                  </div>
                  <label className="block">
                    <span className="mb-1 block text-xs uppercase tracking-wide text-slate-500">Description</span>
                    <textarea className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" rows={4} value={workflowTypeForm.description} onChange={e => setWorkflowTypeForm(form => ({ ...form, description: e.target.value }))} />
                  </label>
                  <Button variant="primary" loading={saving === 'workflow-type'} onClick={saveWorkflowType}>
          ...
          {activeTab === 'task-statuses' && (
            <div className="space-y-4">
              ...
              <TaskStatusesTable
                ...
                workflowTypeKey={selectedWorkflowType.key}
                showNewStatus={showNewStatus}
                onCreated={() => {
                  setShowNewStatus(false);
                  void reloadStatuses(selectedWorkflowType.key);
