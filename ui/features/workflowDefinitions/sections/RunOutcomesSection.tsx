...
import type { WorkflowTypeOutcome } from '@/lib/api';
...
export function RunOutcomesTable({
  ...
  onEdit: (outcome: WorkflowTypeOutcome) => void;
  onDelete: (outcome: WorkflowTypeOutcome) => void;
  saving: string | null;
}) {
  ...
  const scopeForOutcome = (outcome: WorkflowTypeOutcome) => outcome.task_type ?? 'base';
  const metadataForOutcome = (outcome: WorkflowTypeOutcome) => {
