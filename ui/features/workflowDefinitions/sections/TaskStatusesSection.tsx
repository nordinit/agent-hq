...
function WorkflowDefinitionStatusRow({
  workflowTypeKey,
  status,
  onSaved,
}: {
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      await api.updateWorkflowTypeStatus(workflowTypeKey, status.name, { label, color, emoji: normalizeTaskStatusEmojiInput(emoji) ?? '' });
      setEditing(false);
      onSaved();
    } catch (err) {
  ...
  const deleteStatus = async () => {
    ...
    setError(null);
    try {
      await api.deleteWorkflowTypeStatus(workflowTypeKey, status.name);
...
function NewWorkflowTypeStatusRow({
  workflowTypeKey,
  onCreated,
  onCancel,
}: {
  ...
  const create = async () => {
    ...
    setError(null);
    try {
      await api.createWorkflowTypeStatus(workflowTypeKey, {
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <>
                {showNewStatus && (
                  <NewWorkflowTypeStatusRow workflowTypeKey={workflowTypeKey} onCreated={onCreated} onCancel={onCancelNewStatus} />
                )}
                {filteredStatuses.map(status => (
                  <WorkflowDefinitionStatusRow key={status.name} workflowTypeKey={workflowTypeKey} status={status} onSaved={onSaved} />
                ))}
