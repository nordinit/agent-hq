...
import {
  api, Workflow, WorkflowMetrics, WorkflowType, Task,
} from '@/lib/api';
...
function OverviewTab({ workflow, metrics }: { workflow: Workflow; metrics: WorkflowMetrics | null }) {
  ...
  return (
    <div className="space-y-6">
      <WorkflowTeamCard workflowId={workflow.id} teamId={workflow.team_id ?? null} />
...
function TasksTab({ workflow, tasks, onRefresh }: { workflow: Workflow; tasks: Task[]; onRefresh: () => void }) {
  const base = getApiBase();
  const [selectedTask, setSelectedTask] = useState<Task | null>(null);
  const { statuses: taskStatusCatalog } = useTaskStatuses(workflow.id);
  ...
  return (
    <>
      <TaskBoard
        tasks={tasks}
        storageKey="workflow-tasks-visible-cols"
...
function MetricsTab({ metrics }: { metrics: WorkflowMetrics | null }) {
  if (!metrics) return <div className="text-slate-500 text-sm">Loading metrics…</div>;
...
function EditForm({ workflow, onSave, onCancel }: {
  workflow: Workflow;
  ...
  const [environmentSetup, setEnvironmentSetup] = useState<EnvironmentSetup>(workflow.environment_setup ?? { mode: 'off' });
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        status: form.status as Workflow['status'],
        length_kind: form.length_kind as Workflow['length_kind'],
        length_value: form.length_value,
        started_at: form.started_at || null,
        repo_access_mode: (form.repo_access_mode || null) as Workflow['repo_access_mode'],
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <select className="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-400"
            value={form.workflow_type} onChange={e => set('workflow_type', e.target.value)}>
          ...
          <p className="text-xs text-slate-500 mt-1">
            ...
            {selectedWorkflowType ? ` ${(selectedWorkflowType.description ?? '')}` : ''}
          </p>
        </div>
        <div>
...
export default function WorkflowDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const s = await api.getWorkflow(id);
      const [m, t] = await Promise.all([
        api.getWorkflowMetrics(id),
        fetch(`${base}/api/v1/tasks?workflow_id=${id}`).then(r => r.json()),
      ]);
      setWorkflow(s);
      setMetrics(m);
      setTasks(t as Task[]);
  ...
  // Live polling - silently refresh workflow data every 10s so agent/system-driven changes appear automatically
  useLiveRefresh(load, {
    enabled: Boolean(id),
    intervalMs: 10000,
  ...
  const handlePause = async () => {
    try {
      await api.updateWorkflow(id, { status: 'paused' });
      await load();
  ...
  const handleResume = async () => {
    try {
      await api.updateWorkflow(id, { status: 'active' });
      await load();
    } catch (e) { alert(String(e)); }
  };
  ...
  const workflowTypeLabel = formatWorkflowTypeLabel(workflow.workflow_type);
  ...
  return (
    <div className={`flex flex-col ${tab === 'tasks' ? 'md:h-full md:overflow-hidden p-4 md:p-6' : 'space-y-6'}`}>
      ...
      <div className={`flex items-center gap-2 text-sm text-slate-500 ${tab === 'tasks' ? 'mb-4 flex-shrink-0' : ''}`}>
        ...
        <span>/</span>
        <span className="text-slate-300">{formatWorkflowLabel(workflow)}</span>
      </div>

      {/* Header */}
      <div className={tab === 'tasks' ? 'flex-shrink-0 mb-4' : ''}>
      {editing ? (
        <EditForm workflow={workflow} onSave={handleSave} onCancel={() => setEditing(false)} />
      ) : (
        <Card>
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <h1 className="text-xl font-bold text-white">{formatWorkflowLabel(workflow)}</h1>
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_BADGE[workflow.status]}`}>
                  {workflow.status}
                </span>
                {workflow.project_name && (
                  <Badge variant="workspace">{workflow.project_name}</Badge>
                )}
                {workflowTypeLabel && (
              ...
              <div className="flex items-center gap-4 mt-3 text-xs text-slate-500 flex-wrap">
                ...
                {workflow.project_name && (
                  <span>Project: {workflow.project_name}</span>
                )}
                {workflowTypeLabel && (
                  <span>Workflow Type: {workflowTypeLabel}</span>
                )}
                {workflow.repo_access_mode === 'worktree' && workflow.repo_path && (
                  <span className="font-mono text-emerald-300 break-all">Repo: {workflow.repo_path}</span>
                )}
                {workflow.repo_access_mode === 'clone' && workflow.repo_url && (
                  <span className="font-mono text-cyan-300 break-all">Repo: {workflow.repo_url}</span>
                )}
                ...
                {workflow.ended_at && (
                  <span>Ended: {formatDate(workflow.ended_at)}</span>
                )}
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end shrink-0">
              ...
              {workflow.status !== 'complete' && workflow.status !== 'closed' && (
      ...
      <div className={tab === 'tasks' ? 'md:flex-1 md:min-h-0 flex flex-col md:overflow-hidden' : ''}>
        {tab === 'overview' && <OverviewTab workflow={workflow} metrics={metrics} />}
        {tab === 'tasks' && <TasksTab workflow={workflow} tasks={tasks} onRefresh={load} />}
        {tab === 'files' && <ProjectFiles projectId={workflow.project_id} workflowId={workflow.id} scope="workflow" />}
        {tab === 'metrics' && <div className="space-y-5"><Card><TelemetryScopeSummary scope={{ project_id: workflow.project_id, workflow_id: workflow.id, workflow_type: workflow.workflow_type ?? undefined }} /></Card><details><summary className="cursor-pointer text-sm text-slate-500">Legacy workflow summary</summary><div className="mt-4"><MetricsTab metrics={metrics} /></div></details></div>}
      </div>
    </div>
  );
}
