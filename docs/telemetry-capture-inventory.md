...
| `task_field_schemas` | `domains/sprint-definitions/router.ts`, starter/default install/import paths. | Effective field descriptors include schema ID and content fingerprint. Old observations retain their descriptors when a field is renamed, removed or changes type. |
| Configuration tables | `sprint_task_{routing_rules,statuses,transition_requirements,transitions}`, `sprint_type_{outcomes,relationship_types,task_statuses,task_types}`, `sprint_types`, `external_event_mappings`, `story_point_model_routing`, `routing_config`, `routing_transitions`, `routing_config_audit_log`, `agents`, `projects`, `sprints`. | Explicit safe configuration keys and audit fingerprints; no arbitrary metadata, credentials or instructions. Agent instructions are represented by fingerprints and version. Execution attribution uses execution-time observations, never current agent configuration from a later callback. |
