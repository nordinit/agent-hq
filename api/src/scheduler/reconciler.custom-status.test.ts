...
function setupDb(): Database.Database {
  ...
  `);
  db.prepare(`INSERT INTO sprints (id, project_id, name, goal, sprint_type, status) VALUES (10, 86, 'Elevation Built', 'Goal', 'dev', 'active')`).run();
  db.prepare(`INSERT INTO agents (id, name, job_title, job_instructions, enabled, timeout_seconds, session_key, runtime_type) VALUES (1, 'Old Agent', 'Old', 'Old', 1, 900, 'agent:old', 'openclaw')`).run();
  db.prepare(`INSERT INTO agents (id, name, job_title, job_instructions, enabled, timeout_seconds, session_key, runtime_type) VALUES (2, 'Addison', 'PM', 'Review', 1, 900, 'agent:addison', 'openclaw')`).run();
  return db;
}

function insertTask(db: Database.Database, id: number, status: string): void {
  db.prepare(`
    INSERT INTO tasks (
...
describe('reconciler workflow-defined status routing', () => {
  it('reconciles ownership for a custom status with a matching routing rule', async () => {
    ...
    const task = db.prepare(`SELECT assigned_agent_id FROM tasks WHERE id = 797`).get() as { assigned_agent_id: number };
  ...
  it('does not reconcile ownership for workflow terminal custom statuses', async () => {
    const db = setupDb();
    db.prepare(`INSERT INTO sprint_task_statuses (sprint_id, status_key, label, terminal) VALUES (10, 'archived', 'Archived', 1)`).run();
    db.prepare(`
      INSERT INTO sprint_task_routing_rules (sprint_id, project_id, sprint_type, task_type, status, agent_id, priority)
      VALUES (10, 86, 'dev', 'backend', 'archived', 2, 10)
    `).run();
    insertTask(db, 798, 'archived');
