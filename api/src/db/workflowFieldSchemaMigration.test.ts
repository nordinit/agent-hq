...
import { resolveWorkflowMetadata } from '../domains/workflow-definitions/workflowMetadata';
import { seedWorkflowTaskPolicy } from '../domains/routing/policy';
...
async function seedStarterDefinitions(db: Db): Promise<number> {
  ...
  if (!install.installed) {
    throw new Error('workflow field schema fixture expected a fresh explicit installation');
  }
  return await ensureTenantSchema(db);
}

describe('workflow field schema seeding', () => {
  let workspaceRoot = '';

  beforeEach(async () => {
    // Keep any explicitly installed workspace files out of the developer's runtime directory.
    workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'workflow-field-schema-ws-'));
  ...
  it('seeds dev workflow fields as unified task fields', async () => {
    const db = getDb();
    ...
    const row = await db.get(`
      ...
      WHERE workflow_type_key = 'dev' AND task_type IS NULL
      LIMIT 1
  ...
  it('resolves dev task workflow metadata from workflow task policy', async () => {
    const db = getDb();
    ...
    `, tenantId);
    await db.run(`
      INSERT INTO workflows (id, tenant_id, project_id, name, goal, workflow_type, status, length_kind, length_value, created_at)
      VALUES (9101, ?, 910, 'Dev Workflow', '', 'dev', 'active', 'time', '2w', CURRENT_TIMESTAMP)
    `, tenantId);
    await seedWorkflowTaskPolicy(db, 9101);

    const metadata = await resolveWorkflowMetadata(db, { workflowId: 9101, taskType: 'backend' });
  ...
  it('filters stale starter transitions out of custom workflow metadata readbacks', async () => {
    ...
    await db.run(`
      INSERT INTO workflow_task_transitions (
        tenant_id, workflow_id, project_id, workflow_type, task_type, from_status, outcome, to_status, enabled, priority, is_protected, created_at, updated_at
      ) VALUES
    ...
    const metadata = await resolveWorkflowMetadata(db, { workflowId: 9901, taskType: 'lead' });
  ...
  it('seeds only generic, dev, and ops workflow types', async () => {
    const db = getDb();
    await seedStarterDefinitions(db);
  ...
  it('seeds ops as a distinct operational workflow', async () => {
    ...
    const opsStatuses = await db.all(`
      ...
      WHERE tenant_id = ? AND workflow_type_key = 'ops'
    ...
    await db.run(`
      INSERT INTO workflows (id, tenant_id, project_id, name, goal, workflow_type, status, length_kind, length_value, created_at)
      VALUES (9201, ?, 920, 'Ops Workflow', '', 'ops', 'active', 'time', '2w', CURRENT_TIMESTAMP)
    `, tenantId);
    await seedWorkflowTaskPolicy(db, 9201);

    const metadata = await resolveWorkflowMetadata(db, { workflowId: 9201, taskType: 'ops' });
    expect(metadata.statuses.map(status => status.name)).toEqual([
      'todo',
      'intake',
      'triage',
