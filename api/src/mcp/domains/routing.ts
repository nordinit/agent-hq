...
export function registerRoutingTools(ctx: McpDomainContext) {
  ...
  const graphScopeSchema = {
    ...
    workflow_id: z.number().int().positive().optional().describe('Workflow ID, to layer workflow-specific overrides over the type defaults'),
    workflow_type: z.string().min(1).describe('Workflow type key. Required: a graph is always scoped to one workflow type'),
  ...
  const previewOperationSchema = z.object({
    ...
    payload: z.record(z.string(), z.unknown()).describe('The same body the corresponding write endpoint takes, including its own project_id/workflow_type/workflow_id scope. Scope is NOT inherited from the envelope: a payload without it is rejected'),
  });
  ...
  registerTool(
    ...
    'Cost a routing config change before making it. Applies the real mutations inside a transaction that never commits, then reports rows written per table, how many workflows the change reaches, and the lint findings it introduces or resolves. Use it before every create, update or delete of a transition, assignment rule or gate requirement. The lint delta is the reason it exists: gate resolution replaces rather than accumulates, so a task-type gate substitutes the whole all-types set for that type instead of adding to it, and this is the only place that shows it before the change lands. Requires workflow.edit_routing_config.',
    {
      ...
      workflow_type: z.string().min(1).optional().describe('Workflow type key the change is scoped to'),
      workflow_id: z.number().int().positive().optional().describe('Workflow ID, when the change is a workflow override rather than a type default'),
  ...
  registerTool(
    ...
    'Read the routing config audit trail, newest first. Each entry records who changed which row, the action, a per-field diff, and how many workflows the change reached. Use it to answer "why does this workflow behave differently than it did last week" without diffing config by hand, and to check whether a change you are about to make undoes someone else\'s. Requires workflow.edit_routing_config.',
    {
      ...
      workflow_id: z.number().int().positive().optional().describe('Filter to one workflow'),
      workflow_type: z.string().min(1).optional().describe('Filter to one workflow type'),
      entity_table: z.string().min(1).optional().describe('Filter to one table, e.g. workflow_task_transition_requirements'),
  ...
  registerTool(
    ...
    'List assignment rules that map task type and status to agents for workflow-type defaults or workflow overrides. Non-admin MCP keys require routing_rules.read_project_scope or routing_rules.manage_project_scope and are limited to their assigned project. Optional tenant_id is super-admin MCP only.',
    {
      ...tenantSelectorSchema,
      ...
      scope: z.enum(['defaults', 'overrides', 'workflow_type_default', 'workflow_override']).optional().describe('Optional scope filter'),
  ...
  registerTool(
    ...
    'Get a single assignment rule by ID within its scoped project/workflow-type context. Non-admin MCP keys require routing_rules.read_project_scope or routing_rules.manage_project_scope and are limited to their assigned project. Optional tenant_id is super-admin MCP only.',
    {
  ...
  registerTool(
    ...
    {
      ...tenantSelectorSchema,
      workflow_id: z.number().int().positive().optional().describe('Workflow ID for workflow-specific overrides'),
      project_id: z.number().int().positive().optional().describe('Project ID'),
      workflow_type: z.string().min(1).optional().describe('Workflow type key for defaults or scoped overrides'),
      ...
      scope_kind: z.enum(['workflow_type_default', 'workflow_override']).optional().describe('Explicit rule scope'),
  ...
  registerTool(
    ...
    'Delete a workflow-type default or workflow-specific assignment rule. Non-admin MCP keys require routing_rules.manage_project_scope and can only delete rules in their assigned project. Optional tenant_id is super-admin MCP only.',
    {
      rule_id: z.number().int().positive().describe('Assignment rule ID'),
      ...tenantSelectorSchema,
      project_id: z.number().int().positive().optional().describe('Project ID'),
  ...
  registerTool(
    ...
    {
      ...tenantSelectorSchema,
      workflow_id: z.number().int().positive().optional().describe('Workflow override scope'),
      project_id: z.number().int().positive().describe('Required project scope'),
      workflow_type: z.string().optional().describe('Workflow type for default scope'),
    },
  ...
  registerTool(
    ...
    'Create an automatic workflow transition for a workflow-type default or workflow-specific override. Non-admin MCP keys require routing_transitions.manage_project_scope and can only create transitions in their assigned project. Optional tenant_id is super-admin MCP only.',
    {
      ...
      is_protected: z.boolean().optional().describe('Protected flag for workflow-scoped rules'),
  ...
  registerTool(
    ...
    'List story-point model-routing rules, optionally scoped to a project, workflow, or workflow type.',
    {
      project_id: z.number().int().positive().optional().describe('Project scope to list'),
      workflow_id: z.number().int().positive().optional().describe('Workflow scope to list'),
      workflow_type: z.string().min(1).optional().describe('Workflow-type scope to list'),
    },
  ...
  registerTool(
    ...
    'Create a story-point model-routing rule.',
    {
      ...
      workflow_id: z.number().int().positive().nullable().optional().describe('Workflow scope'),
      workflow_type: z.string().min(1).nullable().optional().describe('Workflow-type scope'),
    },
