import { z } from 'zod';
import { dashboardSchema } from '../../domains/telemetry/dashboards';
import type { McpDomainContext } from '../registrar';
import { telemetryMetricSchema, telemetryProfileSchema, telemetryReportSchema, telemetryPredicateSchema, telemetryValueSchema } from '../../domains/telemetry/inputSchemas';
...
const queryFields = {
  definition: telemetryMetricSchema.optional().describe('Draft metric definition; choose exactly one of definition, metric_revision_id, report_revision_id, or family_key. The catalog definition_contract includes full schemas and examples.'),
  ...
  group_by: z.array(telemetryValueSchema).max(3).optional().describe('At most three catalog field/value expressions; applies consistently to the result and contributors. Use {field:"agent_id"} to group by the metric attribution policy.'),
  bucket: z.enum(['hour','day','week','month']).nullable().optional().describe('Calendar bucket override for a saved historical metric. Null removes its time bucket. Current snapshots cannot be time series.'),
  filter: telemetryPredicateSchema.optional().describe('An additional narrowing predicate; cannot broaden authorized project scope. Task-title regex: {field:"title",op:"matches_regex",value:"^(Lead|Proposal):",flags:"i"}. Patterns use RE2 syntax; flags i/m/s are optional. Put the predicate in definition.population to save it with a metric. Use explicit predicates, not signal_ref, in overrides.'),
...
export function registerTelemetryTools(ctx: McpDomainContext): void {
  ...
  registerTool(['agent_hq_list_telemetry_catalog'], 'Read this before building telemetry. Returns authorized canonical fields, workflow signals, recipes, scope choices and historical availability, plus definition_contract: self-contained JSON schemas and examples for metrics, population filters, formulas, attribution, journeys, profiles, saved views and dashboard widgets. Business success and blockage are configured, never inferred from status labels. Requires telemetry.read.', scopeFields,
  ...
  registerTool(['agent_hq_validate_telemetry_definition'], 'Validate a draft metric, canonical field references, scoped profile bindings and bounded formulas without calculating or activating it. Requires telemetry.query.',
    { definition: telemetryMetricSchema, scope: scope.optional(), profile_revision_id: id.optional() }, args => wrap(() => api.telemetryWrite('POST', '/definitions/validate', args))(), options('/definitions/validate'));
  ...
  registerTool(['agent_hq_get_telemetry_contributors'], 'Inspect the records and inclusion/exclusion explanations retained for a specific calculation. Contributors use the original pinned values and time boundary while rechecking current access. Requires telemetry.read.',
    { query_id: id, offset: z.number().int().min(0).optional(), limit: z.number().int().min(1).max(200).optional(), metric_revision_id: id.optional(), metric_index: z.number().int().min(0).optional(), included: z.boolean().optional(), group: z.string().max(4096).optional().describe('Exact result group key encoded as a JSON array, for example [99974437] or [null]. Filters before pagination.') },
    ({ query_id, ...params }) => wrap(() => api.telemetryGet(`/queries/${pathId(query_id)}/contributors`, params))(), options('/queries/:id/contributors'));

  for (const [collection, singular, capability] of [['metrics', 'metric', 'telemetry.manage_metrics'], ['profiles', 'profile', 'telemetry.manage_metrics'], ['reports', 'report', 'telemetry.manage_reports'], ['dashboards', 'dashboard', 'telemetry.manage_reports']] as const) {
    const definitionSchema = singular === 'metric' ? telemetryMetricSchema : singular === 'profile' ? telemetryProfileSchema : singular === 'dashboard' ? dashboardSchema : telemetryReportSchema;
    ...
    registerTool([`agent_hq_save_telemetry_${singular}`], `Save a new named telemetry ${singular} with an immutable initial revision. ${singular === 'dashboard' ? 'Versioned dashboard page: appearance, sections, weighted columns, typed blocks, and up to 10 pinned metric bindings. Data calculations are separate from page layout.' : singular === 'profile' ? 'Profile definition is {signals:{name:predicate}}.' : singular === 'report' ? 'Reports pin metric revisions. Set presentation to view for a saved single-metric view, or omit it for a multi-metric report (up to 10). Do not build dashboards here: use agent_hq_save_telemetry_dashboard, since the UI only shows dashboard pages. Entries support id, metric_id, metric_revision_id, title, display (card/table/bar/line/funnel/distribution) and view (group_by, bucket, filter, scope, from, to, timezone, sort). Line charts require historical metrics and a bucket; snapshot counts cannot have historical trends.' : 'Metric definition uses the catalog expression contract; success, rework and denominator remain explicit.'} Requires ${capability}.`,
      { key: z.string().min(1).max(128), name: z.string().min(1).max(200), description: z.string().max(4000).optional(), scope: scope.optional(), definition: definitionSchema, ...(singular === 'metric' ? { profile_revision_id: id.optional() } : {}) },
    ...
    registerTool([`agent_hq_revise_telemetry_${singular}`], `Create an immutable new revision of a telemetry ${singular}. Supply the current expected revision to prevent lost updates. Existing pinned reports/bindings remain on their selected versions. Requires ${capability}.`,
      { definition_id: id, expected_revision_id: id, definition: definitionSchema, name: z.string().min(1).max(200).optional(), description: z.string().max(4000).optional(), ...(singular === 'metric' ? { profile_revision_id: id.optional() } : {}) },
  ...
  registerTool(['agent_hq_freeze_telemetry_report'], 'Freeze a completed calculation of a saved report using its pinned definitions and retained contributor evidence. Snapshot expiry follows operator retention settings. Requires telemetry.manage_reports.',
    { report_id: id, query_id: id, report_revision_id: id.optional().describe('Expected pinned report revision for this completed query. A mismatch is rejected before freezing; omission preserves existing behavior.') }, ({ report_id, ...body }) => wrap(() => api.telemetryWrite('POST', `/reports/${pathId(report_id)}/snapshots`, body))(), options('/reports/:id/snapshots'));
  ...
  registerTool(['agent_hq_export_telemetry_definitions'], 'Export selected authorized metrics, profiles and reports with their dependencies for explicit remapping to another workflow/project. Does not export task records. Requires telemetry.export.',
    { scope: scope.optional(), metric_ids: z.array(id).max(100).optional(), report_ids: z.array(id).max(100).optional(), dashboard_ids: z.array(id).max(100).optional(), profile_ids: z.array(id).max(100).optional() },
