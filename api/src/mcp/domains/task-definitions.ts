...
import {
  ...
  WORKFLOW_TYPE_OUTCOME_BEHAVIORS,
  TASK_FIELD_TYPES,
...
export function registerTaskDefinitionsTools(ctx: McpDomainContext) {
  ...
  registerTool(
    ...
    ({ key, ...params }) => wrap(() => api.getWorkflowType(key, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key', '/api/v1/workflow-definitions/types/:key'] },
  );
  ...
  registerTool(
    ...
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), ...tenantSelectorSchema },
    ({ workflow_type_key, ...params }) => wrap(() => api.listWorkflowTypeTaskTypes(workflow_type_key, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/task-types', '/api/v1/task-definitions/workflow-types/:key/task-types'] },
  );
  ...
  registerTool(
    ...
    {
      workflow_type_key: z.string().min(1).describe('Workflow type key'),
      task_types: z.array(z.string().min(1)).describe('Allowed task type keys'),
    },
    ({ workflow_type_key, task_types }) => wrap(() => api.updateWorkflowTypeTaskTypes(workflow_type_key, task_types))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/task-types', '/api/v1/task-definitions/workflow-types/:key/task-types'] },
  );
  ...
  registerTool(
    ...
    (args) => wrap(() => api.createWorkflowType(args))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types', '/api/v1/task-definitions/workflow-types'] },
  );
  ...
  registerTool(
    ...
    ({ key, ...patch }) => wrap(() => api.updateWorkflowType(key, patch))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key', '/api/v1/task-definitions/workflow-types/:key'] },
  );
  ...
  registerTool(
    ...
    ({ key, ...params }) => wrap(() => api.deleteWorkflowType(key, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key', '/api/v1/task-definitions/workflow-types/:key'] },
  );
  ...
  const transitionRequirementSchema = z.object({
    ...
    workflow_type: z.string().min(1).optional().describe('Optional workflow type scope for default requirements'),
  ...
  const workflowTypeStatusSchema = {
  ...
  const workflowTypeOutcomeSchema = {
    ...
    enabled: z.boolean().optional().describe('Enabled flag'),
    behavior: z.enum(WORKFLOW_TYPE_OUTCOME_BEHAVIORS).optional().describe('Outcome merge behavior'),
  ...
  const workflowTypeRelationshipTypeSchema = {
  ...
  registerTool(
    ...
    {
      workflow_type: z.string().min(1).optional().describe('Workflow type key'),
      workflow_type_key: z.string().min(1).optional().describe('Workflow type key alias'),
    },
  ...
  registerTool(
    ...
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/config'] },
  );
  ...
  registerTool(
    ...
    {
      ...tenantSelectorSchema,
      workflow_id: z.number().int().positive().optional().describe('Optional workflow scope'),
      workflow_type: z.string().min(1).optional().describe('Optional workflow type scope'),
      task_type: taskTypeSchema.optional().describe('Optional task type scope'),
    },
    (args) => wrap(() => api.getWorkflowMetadata(args))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/workflow-metadata'] },
  );
  ...
  registerTool(
    ...
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), ...tenantSelectorSchema },
    ({ workflow_type_key, ...params }) => wrap(() => api.listWorkflowTypeStatuses(workflow_type_key, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/statuses'] },
  );
  ...
  registerTool(
    ...
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), status_key: z.string().min(1).describe('Status key'), ...tenantSelectorSchema },
    ({ workflow_type_key, status_key, ...params }) => wrap(() => api.getWorkflowTypeStatus(workflow_type_key, status_key, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/statuses/:statusKey'] },
  );

  registerTool(
    ['agent_hq_create_workflow_type_status'],
    'Create a workflow definition status label.',
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), ...workflowTypeStatusSchema, name: z.string().min(1).describe('Status key'), label: z.string().min(1).describe('Visible status label') },
    ({ workflow_type_key, ...payload }) => wrap(() => api.createWorkflowTypeStatus(workflow_type_key, payload))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/statuses'] },
  );

  registerTool(
    ['agent_hq_update_workflow_type_status'],
    'Update a workflow definition status label.',
    { ...workflowTypeStatusSchema, workflow_type_key: z.string().min(1).describe('Workflow type key'), status_key: z.string().min(1).describe('Current status key') },
    ({ workflow_type_key, status_key, ...patch }) => wrap(() => api.updateWorkflowTypeStatus(workflow_type_key, status_key, patch))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/statuses/:statusKey'] },
  );

  registerTool(
    ['agent_hq_delete_workflow_type_status'],
    'Delete a workflow definition status label.',
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), status_key: z.string().min(1).describe('Status key') },
    ({ workflow_type_key, status_key }) => wrap(() => api.deleteWorkflowTypeStatus(workflow_type_key, status_key))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/statuses/:statusKey'] },
  );
  ...
  registerTool(
    ...
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), ...tenantSelectorSchema },
    ({ workflow_type_key, ...params }) => wrap(() => api.listWorkflowTypeOutcomes(workflow_type_key, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/outcomes'] },
  );
  ...
  registerTool(
    ...
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), outcome_id: z.number().int().positive().describe('Outcome definition ID'), ...tenantSelectorSchema },
    ({ workflow_type_key, outcome_id, ...params }) => wrap(() => api.getWorkflowTypeOutcome(workflow_type_key, outcome_id, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/outcomes/:outcomeId'] },
  );

  registerTool(
    ['agent_hq_create_workflow_type_outcome'],
    'Create a workflow definition run outcome.',
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), ...workflowTypeOutcomeSchema, outcome_key: z.string().min(1).describe('Outcome key'), label: z.string().min(1).describe('Visible outcome label') },
    ({ workflow_type_key, ...payload }) => wrap(() => api.createWorkflowTypeOutcome(workflow_type_key, payload))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/outcomes'] },
  );

  registerTool(
    ['agent_hq_update_workflow_type_outcome'],
    'Update a workflow definition run outcome.',
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), outcome_id: z.number().int().positive().describe('Outcome definition ID'), ...workflowTypeOutcomeSchema },
    ({ workflow_type_key, outcome_id, ...patch }) => wrap(() => api.updateWorkflowTypeOutcome(workflow_type_key, outcome_id, patch))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/outcomes/:outcomeId'] },
  );

  registerTool(
    ['agent_hq_delete_workflow_type_outcome'],
    'Delete a workflow definition run outcome.',
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), outcome_id: z.number().int().positive().describe('Outcome definition ID') },
    ({ workflow_type_key, outcome_id }) => wrap(() => api.deleteWorkflowTypeOutcome(workflow_type_key, outcome_id))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/outcomes/:outcomeId'] },
  );
  ...
  registerTool(
    ...
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), ...tenantSelectorSchema },
    ({ workflow_type_key, ...params }) => wrap(() => api.listWorkflowTypeRelationshipTypes(workflow_type_key, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/relationship-types'] },
  );
  ...
  registerTool(
    ...
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), relationship_type_id: z.number().int().positive().describe('Relationship type ID'), ...tenantSelectorSchema },
    ({ workflow_type_key, relationship_type_id, ...params }) => wrap(() => api.getWorkflowTypeRelationshipType(workflow_type_key, relationship_type_id, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/relationship-types/:relationshipTypeId'] },
  );

  registerTool(
    ['agent_hq_create_workflow_type_relationship_type'],
    'Create a workflow definition relationship type.',
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), ...workflowTypeRelationshipTypeSchema, key: z.string().min(1).describe('Relationship type key'), label: z.string().min(1).describe('Forward label') },
    ({ workflow_type_key, ...payload }) => wrap(() => api.createWorkflowTypeRelationshipType(workflow_type_key, payload))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/relationship-types'] },
  );

  registerTool(
    ['agent_hq_update_workflow_type_relationship_type'],
    'Update a workflow definition relationship type.',
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), relationship_type_id: z.number().int().positive().describe('Relationship type ID'), ...workflowTypeRelationshipTypeSchema },
    ({ workflow_type_key, relationship_type_id, ...patch }) => wrap(() => api.updateWorkflowTypeRelationshipType(workflow_type_key, relationship_type_id, patch))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/relationship-types/:relationshipTypeId'] },
  );

  registerTool(
    ['agent_hq_delete_workflow_type_relationship_type'],
    'Delete a workflow definition relationship type.',
    { workflow_type_key: z.string().min(1).describe('Workflow type key'), relationship_type_id: z.number().int().positive().describe('Relationship type ID') },
    ({ workflow_type_key, relationship_type_id }) => wrap(() => api.deleteWorkflowTypeRelationshipType(workflow_type_key, relationship_type_id))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/relationship-types/:relationshipTypeId'] },
  );
  ...
  registerTool(
    ...
    ({ workflow_type_key, ...params }) => wrap(() => api.listTaskFieldSchemas(workflow_type_key, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/field-schemas', '/api/v1/task-definitions/workflow-types/:key/field-schemas'] },
  );
  ...
  registerTool(
    ...
    ({ workflow_type_key, schema_id, ...params }) => wrap(() => api.getTaskFieldSchema(workflow_type_key, schema_id, params))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/field-schemas/:schemaId', '/api/v1/task-definitions/workflow-types/:key/field-schemas/:schemaId'] },
  );
  ...
  registerTool(
    ...
    ({ workflow_type_key, ...payload }) => wrap(() => api.createTaskFieldSchema(workflow_type_key, payload))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/field-schemas', '/api/v1/task-definitions/workflow-types/:key/field-schemas'] },
  );
  ...
  registerTool(
    ...
    ({ workflow_type_key, schema_id, ...payload }) => wrap(() => api.updateTaskFieldSchema(workflow_type_key, schema_id, payload))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/field-schemas/:schemaId', '/api/v1/task-definitions/workflow-types/:key/field-schemas/:schemaId'] },
  );
  ...
  registerTool(
    ...
    {
      ...tenantSelectorSchema,
      workflow_id: z.number().int().positive().optional().describe('Optional workflow-scoped requirement set'),
      project_id: z.number().int().positive().optional().describe('Optional project scope for workflow-type defaults'),
      workflow_type: z.string().min(1).optional().describe('Optional workflow type scope for default requirements'),
  ...
  registerTool(
    ...
    'Update a workflow gate requirement row. Non-admin MCP keys require transition_requirements.manage_project_scope and can only update requirements inside their assigned project with explicit project/workflow scope. Optional tenant_id is super-admin MCP only. This edits the real configurable gate behavior used by task outcomes.',
    {
      ...
      ...tenantSelectorSchema,
      workflow_id: z.number().int().positive().optional().describe('Optional workflow scope for workflow-specific overrides'),
      project_id: z.number().int().positive().optional().describe('Optional project scope for workflow-type defaults'),
      workflow_type: z.string().min(1).optional().describe('Optional workflow type scope for default requirements'),
      patch: transitionRequirementSchema.partial().describe('Partial requirement update payload'),
      dry_run: z.boolean().optional().describe('Preview validation and affected transition requirement row without writing config'),
    },
    ({ requirement_id, tenant_id, workflow_id, project_id, workflow_type, patch, dry_run }) => wrap(() => api.updateTransitionRequirement(requirement_id, { ...(patch ?? {}), ...(tenant_id !== undefined ? { tenant_id } : {}), ...(workflow_id !== undefined ? { workflow_id } : {}), ...(project_id !== undefined ? { project_id } : {}), ...(workflow_type !== undefined ? { workflow_type } : {}), ...(dry_run !== undefined ? { dry_run } : {}) }))(),
  ...
  registerTool(
    ...
    ({ workflow_type_key, schema_id }) => wrap(() => api.deleteTaskFieldSchema(workflow_type_key, schema_id))(),
    { domain: 'task_definitions', rest_paths: ['/api/v1/workflows/types/:key/field-schemas/:schemaId', '/api/v1/task-definitions/workflow-types/:key/field-schemas/:schemaId'] },
  );
}
