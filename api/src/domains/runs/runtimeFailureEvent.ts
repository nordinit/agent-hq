...
export async function applyConfiguredRuntimeFailedEvent(
  ...
  writeTaskHistory(db, params.taskId, changedBy, 'workflow_event_source', null, AGENT_HQ_RUNTIME_SOURCE, false);
  writeTaskHistory(db, params.taskId, changedBy, 'workflow_event_name', null, RUNTIME_FAILED_EVENT, false);
  writeTaskHistory(db, params.taskId, changedBy, 'workflow_event_runtime_end_source', null, params.runtimeEndSource, false);
  writeTaskHistory(db, params.taskId, changedBy, 'workflow_event_instance_id', null, params.instanceId, false);
  writeTaskHistory(db, params.taskId, changedBy, 'workflow_event_mapping_id', null, mapping?.id ?? null, false);
  writeTaskHistory(db, params.taskId, changedBy, 'workflow_event_action_kind', null, mapping?.action_kind ?? null, false);
  writeTaskHistory(db, params.taskId, changedBy, 'workflow_event_action_target', null, mapping?.action_target ?? null, false);

  try {
    db.prepare(`INSERT INTO task_notes (task_id, author, content) VALUES (?, ?, ?)`).run(params.taskId, changedBy, [
  ...
  if (mapping.action_kind === 'status' && mapping.action_target) {
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(mapping.action_target, mapping.apply_failure_detail ? 1 : 0, failureDetail, params.taskId);
    writeTaskStatusChange(db, params.taskId, changedBy, params.priorTaskStatus, mapping.action_target, {
