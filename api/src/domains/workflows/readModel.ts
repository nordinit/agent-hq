...
async function workflowRoutingAgentCountPredicate(db: Db): Promise<string> {
  return await tableHasColumn(db, 'workflow_task_routing_rules', 'project_id')
    ...
    ? `rr2.workflow_id = s.id
           OR (
             rr2.project_id = s.project_id
             AND rr2.workflow_type = s.workflow_type
             AND rr2.workflow_id IS NULL
           )`
    : `rr2.workflow_id = s.id`;
}

async function workflowRoutingJobsPredicate(db: Db): Promise<string> {
  return await tableHasColumn(db, 'workflow_task_routing_rules', 'project_id')
    && await tableHasColumn(db, 'workflow_task_routing_rules', 'workflow_type')
    ? `rr.workflow_id = s.id
       OR (
...
export async function listWorkflows(
  ...
  const routingAgentCountPredicate = await workflowRoutingAgentCountPredicate(db);
  ...
  let sql = `
    SELECT s.*, s.id AS workflow_id,
      ...
      (
        ...
        FROM workflow_task_routing_rules rr2
...
export async function getWorkflowDetail(db: Db, workflowId: number) {
  const routingAgentCountPredicate = await workflowRoutingAgentCountPredicate(db);
  return await db.get(`
    SELECT s.*, s.id AS workflow_id,
      p.name as project_name,
...
export async function getWorkflowMetrics(db: Db, workflowId: number) {
  ...
  const taskRow = await db.get(`
    ...
    FROM tasks
    WHERE workflow_id = ?
  `, workflowId) as { tasks_total: number; tasks_done: number; total_story_points: number; done_story_points: number; remaining_story_points: number };
  ...
  const blockerRow = await db.get(`
    ...
    WHERE blocked.workflow_id = ?
      AND blocker.status != 'done'
  `, workflowId) as { blocker_count: number };
  ...
  `, workflowId) as { avg_ms: number | null };
  ...
  const runRow = await db.get(`
    ...
    WHERE t.workflow_id = ?
  `, workflowId) as { job_runs_total: number; job_runs_success: number; job_runs_failed: number };
...
export async function listWorkflowJobs(db: Db, workflowId: number) {
  const workflow = await db.get('SELECT id FROM workflows WHERE id = ?', workflowId);
  if (!workflow) return null;
  const routingJobsPredicate = await workflowRoutingJobsPredicate(db);
    ...
    SELECT a.*,
      ...
      CASE WHEN EXISTS (
        ...
        WHERE rr.agent_id = a.id
          AND rr.workflow_id = ?
      ) THEN 1 ELSE 0 END as is_primary_workflow
    FROM workflow_task_routing_rules rr
    JOIN workflows s ON s.id = ?
