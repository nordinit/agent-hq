...
export async function resolveWorkflowTypeForWorkflowId(db: Db, workflowId: unknown): Promise<string> {
  if (workflowId == null || workflowId === '') return 'generic';

  try {
    const row = await db.get(`SELECT workflow_type FROM workflows WHERE id = ? LIMIT 1`, Number(workflowId)) as { workflow_type?: string | null } | undefined;
...
export async function getAllowedTaskTypesForWorkflowType(db: Db, workflowType: string): Promise<string[]> {
  try {
    ...
    `, normalizeWorkflowType(workflowType)) as Array<{ task_type: string | null }>;
...
export async function isTaskTypeAllowedForWorkflowType(
  ...
  const allowedTaskTypes = await getAllowedTaskTypesForWorkflowType(db, workflowType);
  if (allowedTaskTypes.length === 0) return true;

  return allowedTaskTypes.includes(normalizedTaskType);
}

export async function resolveTaskFieldSchemaForWorkflow(
  db: Db,
  input: { workflowId?: unknown; workflowType?: unknown; taskType?: unknown },
): Promise<ResolvedTaskFieldSchema> {
  const workflowType = input.workflowType != null
    ? normalizeWorkflowType(input.workflowType)
    : await resolveWorkflowTypeForWorkflowId(db, input.workflowId ?? null);
  const taskType = normalizeTaskType(input.taskType);
  const allowedTaskTypes = await getAllowedTaskTypesForWorkflowType(db, workflowType);

  if (!await tableExists(db, 'task_field_schemas')) {
    return { workflow_type: workflowType, schema: { fields: [] }, allowed_task_types: allowedTaskTypes };
  }
  ...
  try {
    const getSchemaForTaskType = async (candidateTaskType: string | null): Promise<{ fields: TaskFieldDefinition[] }> => {
      const row = await db.get(`
        ...
        LIMIT 1
      `, workflowType, candidateTaskType, candidateTaskType) as { schema_json: string } | undefined;
...
export async function validateRequirementFieldExpression(
  db: Db,
  input: { workflowId?: unknown; workflowType?: unknown; taskType?: unknown; fieldName?: unknown; fieldRole?: string },
): Promise<void> {
  const fieldName = typeof input.fieldName === 'string' ? input.fieldName.trim() : '';
  if (!fieldName) return;

  const resolved = await resolveTaskFieldSchemaForWorkflow(db, input);
  ...
  if (unknownFields.length > 0) {
    const err = new Error(`${input.fieldRole ?? 'field_name'} contains field(s) not defined for workflow type "${resolved.workflow_type}": ${unknownFields.join(', ')}`);
...
export async function resolveTaskWorkflowContext(
  db: Db,
  input: { workflowId?: unknown; workflowType?: unknown; taskType?: unknown },
): Promise<{ workflowType: string; taskType: string | null; allowedTaskTypes: string[] }> {
  const workflowType = input.workflowType != null
    ? normalizeWorkflowType(input.workflowType)
    : await resolveWorkflowTypeForWorkflowId(db, input.workflowId ?? null);
  const taskType = normalizeTaskType(input.taskType);
