...
beforeEach(async()=>{
  ...
  await db.run(`INSERT INTO workflow_types(tenant_id,key,name,project_id) VALUES(1,'article','Article',1),(2,'private','Private',2)`);
  await db.run(`INSERT INTO workflows(id,tenant_id,project_id,name,workflow_type) VALUES(1,1,1,'Article','article'),(2,2,2,'Private','private')`);
  await db.run(`INSERT INTO task_field_schemas(tenant_id,workflow_type_key,schema_json) VALUES(1,'article','{"fields":[{"key":"amount","type":"number"}]}')`);
  await db.run(`INSERT INTO workflow_type_task_statuses(id,tenant_id,workflow_type_key,status_key,label) VALUES(1,1,'article','approved','Approved'),(2,1,'article','review','Review')`);
  await db.run(`INSERT INTO workflow_type_outcomes(id,tenant_id,workflow_type_key,outcome_key,label) VALUES(1,1,'article','changes','Changes requested')`);
});
afterEach(async()=>teardownTestDb());
async function catalog(){return getTelemetryCatalog(db,access,scope);}
async function task(id:number,status='approved'){await db.run(`INSERT INTO tasks(id,tenant_id,project_id,workflow_id,title,status,task_type) VALUES(?,1,1,1,?,?, 'article')`,id,`Task ${id}`,status);}
...
it('pins label revisions without retiring live status/outcome identities during catalog reads',async()=>{
  ...
  await db.run(`UPDATE workflow_type_task_statuses SET label='Accepted' WHERE id=1`);
  await db.run(`UPDATE workflow_type_outcomes SET label='Revise' WHERE id=1`);
...
it('new same-key generations cannot reinterpret saved status metrics or a task assigned before retirement',async()=>{
  ...
  await db.run('DELETE FROM workflow_type_task_statuses WHERE id=1');
  await db.run(`INSERT INTO workflow_type_task_statuses(id,tenant_id,workflow_type_key,status_key,label) VALUES(1,1,'article','approved','New approval')`);
...
it('pinned retired status milestones still evaluate their historical observations only',async()=>{
  ...
  await db.run(`UPDATE workflow_type_task_statuses SET status_key='accepted' WHERE id=1`);
  await db.run(`INSERT INTO workflow_type_task_statuses(id,tenant_id,workflow_type_key,status_key,label) VALUES(3,1,'article','approved','Different approved')`);
  await task(2);
  expect((await queryTelemetry(db,access,{metric_revision_id:saved.latest_revision_id})).value).toBe(1);
...
it('accepted outcomes retain their captured identity across retirement and timestamp corrections',async()=>{
  ...
  await db.run('DELETE FROM workflow_type_outcomes WHERE id=1');
  await db.run(`INSERT INTO workflow_type_outcomes(id,tenant_id,workflow_type_key,outcome_key,label) VALUES(1,1,'article','changes','Different meaning')`);
...
it('profile and component references keep the originating revision signal pins',async()=>{
  ...
  await db.run('DELETE FROM workflow_type_task_statuses WHERE id=1');
  await db.run(`INSERT INTO workflow_type_task_statuses(id,tenant_id,workflow_type_key,status_key,label) VALUES(1,1,'article','approved','New')`);
...
it('preserves raw unconfigured status selectors without adopting a later configured meaning',async()=>{
  ...
  await db.run(`INSERT INTO workflow_type_task_statuses(id,tenant_id,workflow_type_key,status_key,label) VALUES(3,1,'article','custom','Configured now')`);
  await task(2,'custom');
  expect((await queryTelemetry(db,access,{metric_revision_id:saved.latest_revision_id})).value).toBe(1);
});
...
it('does not reuse a stale generation when its canonical producer is disabled',async()=>{
  await catalog();
  await db.exec('ALTER TABLE workflow_type_task_statuses DISABLE TRIGGER telemetry_signal_generation');
  try{
    await db.run('DELETE FROM workflow_type_task_statuses WHERE id=1');
    await db.run(`INSERT INTO workflow_type_task_statuses(id,tenant_id,workflow_type_key,status_key,label) VALUES(1,1,'article','approved','Replacement')`);
    await task(1);
    expect(await db.value('SELECT telemetry_status_identity FROM tasks WHERE id=1')).toBeNull();
  }finally{await db.exec('ALTER TABLE workflow_type_task_statuses ENABLE TRIGGER telemetry_signal_generation');}
});

it('rolls back signal retirement atomically and does not rewrite platform run statuses',async()=>{
  const old=(await catalog()).statuses.find(s=>s.key==='approved')!;
  await expect(db.withTransaction(async tx=>{await tx.run('DELETE FROM workflow_type_task_statuses WHERE id=1');throw new Error('abort');})).rejects.toThrow('abort');
...
it('uses workflow overrides and global status lifecycles without leaking tenant-owned signals',async()=>{
  ...
  await db.run(`INSERT INTO workflow_task_statuses(id,workflow_id,status_key,label) VALUES(10,1,'approved','Workflow approval')`);
  await db.run(`INSERT INTO workflow_type_task_statuses(id,tenant_id,workflow_type_key,status_key,label) VALUES(20,2,'private','secret','Secret')`);
  const records=await catalog(),override=records.statuses.find(signal=>signal.source?.table==='workflow_task_statuses')!;
...
it('allows concurrent tenant catalog reads while serializing canonical signal writers',async()=>{
  ...
  try{
    ...
    await expect(db.withTransaction(async tx=>{
      ...
      await tx.run("UPDATE workflow_type_task_statuses SET label='Cannot race read' WHERE id=1");
