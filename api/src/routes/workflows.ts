...
import { attachWorkflowJob, closeWorkflow, completeWorkflowRoute, createWorkflow, deleteWorkflow, detachWorkflowJob, updateWorkflow } from '../domains/workflows/admin';
import { checkWorkflowCompletion } from '../domains/workflows/lifecycle';
import { getWorkflowDetail, getWorkflowMetrics, listWorkflowJobs, listWorkflows } from '../domains/workflows/readModel';
import { resolveTenantIdFromRequest } from '../lib/tenantContext';
import { columnExists as sharedColumnExists } from "../db/introspection";
import { parseIdParam } from '../lib/routeParams';

export { checkWorkflowCompletion };
...
async function requireWorkflowVisibleForTenant(db: ReturnType<typeof getDb>, workflowId: number | string, tenantId: number): Promise<boolean> {
  if (!await routeTableHasColumn(db, 'workflows', 'tenant_id')) {
    return Boolean(await db.get(`SELECT id FROM workflows WHERE id = ?`, workflowId));
  }
  return Boolean(await db.get(`SELECT id FROM workflows WHERE id = ? AND tenant_id = ?`, workflowId, tenantId));
}
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    res.json(await listWorkflows(db, { ...req.query, tenant_id: tenantId }));
  } catch (err) {
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const db = getDb();
    const tenantId = await resolveTenantIdFromRequest(db, req);
    if (!await requireWorkflowVisibleForTenant(db, req.params.id, tenantId)) return res.status(404).json({ error: 'Workflow not found' });
    const workflow = await getWorkflowDetail(db, Number(req.params.id));
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const db = getDb();
    const tenantId = await resolveTenantIdFromRequest(db, req);
    const projectId = Number((req.body as Partial<Workflow>)?.project_id);
    if (Number.isInteger(projectId) && projectId > 0) {
      if (!await requireProjectVisibleForTenant(db, projectId, tenantId)) return res.status(404).json({ error: 'Project not found' });
    }
    return res.status(201).json(await createWorkflow(db, { ...(req.body as Partial<Workflow>), tenant_id: tenantId } as Partial<Workflow>, extractActor(req)));
  } catch (err) {
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    return res.json(await updateWorkflow(db, Number(req.params.id), body, extractActor(req)));
  } catch (err) {
    const typedErr = err as Error & { status?: number; body?: Record<string, unknown> };
    return res.status(typedErr.status ?? 500).json(typedErr.body ?? { error: typedErr.message });
  }
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    const db = getDb();
    const tenantId = await resolveTenantIdFromRequest(db, req);
    return res.json(await deleteWorkflow(db, Number(req.params.id), extractActor(req), tenantId));
  } catch (err) {
...
router.post('/:id/close', async (req: Request, res: Response) => {
  try {
    ...
    const tenantId = await resolveTenantIdFromRequest(db, req);
    if (!await requireWorkflowVisibleForTenant(db, req.params.id, tenantId)) return res.status(404).json({ error: 'Workflow not found' });
    return res.json(await closeWorkflow(db, Number(req.params.id), extractActor(req), lifecycleNote(req)));
  } catch (err) {
    const typedErr = err as Error & { status?: number };
    return res.status(typedErr.status ?? 500).json({ error: typedErr.message });
  }
...
router.post('/:id/complete', async (req: Request, res: Response) => {
  try {
    ...
    const tenantId = await resolveTenantIdFromRequest(db, req);
    if (!await requireWorkflowVisibleForTenant(db, req.params.id, tenantId)) return res.status(404).json({ error: 'Workflow not found' });
    return res.json(await completeWorkflowRoute(db, Number(req.params.id), extractActor(req), lifecycleNote(req)));
  } catch (err) {
    const typedErr = err as Error & { status?: number };
    return res.status(typedErr.status ?? 500).json({ error: typedErr.message });
  }
...
router.get('/:id/metrics', async (req: Request, res: Response) => {
  try {
    ...
    const tenantId = await resolveTenantIdFromRequest(db, req);
    if (!await requireWorkflowVisibleForTenant(db, req.params.id, tenantId)) return res.status(404).json({ error: 'Workflow not found' });
    const metrics = await getWorkflowMetrics(db, Number(req.params.id));
...
router.get('/:id/jobs', async (req: Request, res: Response) => {
  try {
    ...
    const tenantId = await resolveTenantIdFromRequest(db, req);
    if (!await requireWorkflowVisibleForTenant(db, req.params.id, tenantId)) return res.status(404).json({ error: 'Workflow not found' });
    const jobs = await listWorkflowJobs(db, Number(req.params.id));
    if (!jobs) return res.status(404).json({ error: 'Workflow not found' });
...
// ── POST /api/v1/workflows/:id/jobs ────────────────────────────────────────────
// Task #605: workflow-scoped agents are deprecated. Use workflow_task_routing_rules.
router.post('/:id/jobs', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(201).json(await attachWorkflowJob(db, Number(req.params.id), job_id));
  } catch (err) {
    const typedErr = err as Error & { status?: number };
    return res.status(typedErr.status ?? 500).json({ error: typedErr.message });
  }
...
// ── DELETE /api/v1/workflows/:id/jobs/:jobId ───────────────────────────────────
// Task #605: workflow-scoped agents are deprecated. Use workflow_task_routing_rules.
router.delete('/:id/jobs/:jobId', async (req: Request, res: Response) => {
  try {
    ...
    return res.json(await detachWorkflowJob(db, Number(req.params.id), Number(req.params.jobId)));
  } catch (err) {
    const typedErr = err as Error & { status?: number };
    return res.status(typedErr.status ?? 500).json({ error: typedErr.message });
  }
...
// Task #596: workflow_job_schedules table removed. Return empty array for backward compat.
