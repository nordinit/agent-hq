...
async function resetDb(): Promise<void> {
  ...
  await db.run(`INSERT INTO sprint_types (tenant_id, key, name, is_system) VALUES (2, 'generic', 'Generic', 1), (2, 'enhancements', 'Enhancements', 1)`);
  await db.run(`INSERT INTO agents (id, tenant_id, name, role, job_title, system_role, session_key, openclaw_agent_id, model, project_id, sprint_id, enabled) VALUES (7, 2, 'Cinder', 'backend engineer', 'Backend', NULL, 'agent:cinder:main', NULL, NULL, 1, NULL, 1)`);
  await db.run(`
  ...
  `);
  await db.run(`
    INSERT INTO sprint_type_task_statuses (tenant_id, sprint_type_key, status_key, label, color, terminal, is_system, allowed_transitions_json, stage_order, is_default_entry, metadata_json)
...
describe('sprints API create clone support', () => {
  ...
  it('deletes tasks that belong to a deleted workflow', async () => {
    ...
    try {
      const db = getDb();
      await db.run(`INSERT INTO tasks (id, tenant_id, project_id, sprint_id, title, status, story_points) VALUES (120, 2, 1, 10, 'Source task', 'ready', 3), (121, 2, 2, 20, 'Other task', 'ready', 5)`);
  ...
  it('supports workflow metrics, close, and complete endpoint aliases', async () => {
    ...
    try {
      const db = getDb();
      await db.run(`INSERT INTO tasks (id, tenant_id, project_id, sprint_id, title, status, story_points) VALUES (100, 2, 1, 10, 'Done task', 'done', 3), (101, 2, 1, 10, 'Ready task', 'ready', 5), (102, 2, 2, 20, 'Other workflow task', 'ready', 2)`);
  ...
  it('does not install starter statuses when an operator creates a starter-named workflow type', async () => {
    ...
    try {
      ...
      expect(Number(await db.value(`
        SELECT COUNT(*) FROM sprint_type_task_statuses
        WHERE tenant_id = 2 AND sprint_type_key = 'dev'
      `))).toBe(0);

      const response = await fetch(`${baseUrl}/api/v1/sprints/types`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  it('keeps sprint-type status conflict checks even for seeded statuses', async () => {
    ...
    try {
      ...
      `);
      await db.run(`INSERT INTO tasks (tenant_id, project_id, sprint_id, title, status, story_points) VALUES (2, 1, 10, 'Seeded status task', 'seeded_in_use', 3)`);
      await db.run(`
        INSERT INTO sprint_task_transitions (tenant_id, project_id, sprint_id, task_type, from_status, outcome, to_status, enabled, priority, is_protected)
  ...
  it('treats matching workflow-event mappings as an explicit exit path for dispatchable statuses', async () => {
    ...
    try {
      ...
      await db.run(`INSERT INTO sprint_types (tenant_id, key, name, is_system) VALUES (2, 'dev', 'Development', 1)`);
      await db.run(`INSERT INTO sprints (id, tenant_id, project_id, name, sprint_type) VALUES (57, 2, 86, 'Development', 'dev')`);
      ...
      await policy.seedSprintTypeTaskStatuses(db, 'dev', { force: true, tenantId: 2 });
