...
describe('starter routing setup', () => {
  ...
  it('does not provision dev-only routing statuses for a narrowed lead-generation sprint', () => {
    resetDb();
    const db = getDb();
    ...
    syncStarterRoutingForProject(db, 990);
    ...
    `).all() as Array<{ task_type: string; status: string }>;
  ...
  it('preserves ready-to-merge starter routing for dev sprints', () => {
    resetDb();
    const db = getDb();
    const tenantId = defaultTenantId();
    ...
    syncStarterRoutingForProject(db, 991);
