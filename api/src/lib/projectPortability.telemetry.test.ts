...
beforeEach(async()=>{
  ...
  await db.exec(`INSERT INTO tenants(id,name,slug) VALUES(1,'Source','source'),(2,'Destination','destination');
    ...
    INSERT INTO workflow_types(id,tenant_id,project_id,key,name,description,repo_required) VALUES(101,1,101,'delivery','Editorial delivery','Source semantics',1),(202,2,202,'delivery','Private delivery','Do not overwrite',0);
    INSERT INTO workflows(id,tenant_id,project_id,name,workflow_type) VALUES(102,1,101,'Editorial','delivery');
    ...
    INSERT INTO workflow_type_task_types(tenant_id,workflow_type_key,task_type) VALUES(1,'delivery','article'),(2,'delivery','private_task');
    INSERT INTO workflow_type_task_statuses(tenant_id,workflow_type_key,status_key,label,terminal) VALUES(1,'delivery','approved_source','Approved source',1),(2,'delivery','private_status','Private status',1);
    INSERT INTO workflow_task_statuses(workflow_id,status_key,label,terminal) VALUES(102,'submitted_instance','Submitted instance',1);
    INSERT INTO workflow_type_outcomes(tenant_id,workflow_type_key,task_type,outcome_key,label) VALUES(1,'delivery','article','handoff_source','Source handoff');
    INSERT INTO task_field_schemas(id,tenant_id,workflow_type_key,task_type,schema_json) VALUES
...
it('copies schemas and telemetry into a new tenant project with mapped workflow, agent, type and revision identities',async()=>{
  ...
  expect(await db.get('SELECT project_id,description,repo_required FROM workflow_types WHERE tenant_id=2 AND key=?',targetType)).toMatchObject({project_id:result.project_id,description:'Source semantics',repo_required:1});
  expect(await db.value("SELECT description FROM workflow_types WHERE tenant_id=2 AND key='delivery'")).toBe('Do not overwrite');
  expect(await db.get('SELECT tenant_id,workflow_type_key,task_type FROM task_field_schemas WHERE id=?',result.id_map.schemas!['105'])).toEqual({tenant_id:2,workflow_type_key:targetType,task_type:'article'});
  ...
  await db.run("INSERT INTO tasks(tenant_id,project_id,workflow_id,assigned_agent_id,title,status,task_type,custom_fields_json) VALUES(2,?,?,?,'New task','submitted_instance','article','{\"amount\":12}')",result.project_id,targetWorkflow,targetAgent);
...
it.each([false,true])('maps a selected external event mapping to its cloned identity (tenant default: %s)',async tenantDefault=>{
  await db.run("INSERT INTO external_event_mappings(id,tenant_id,project_id,workflow_id,workflow_type,event_name,action_kind,action_target) VALUES(901,1,?,?,?,'submitted','status','submitted_instance')",tenantDefault?null:101,tenantDefault?null:102,tenantDefault?null:'delivery');
  ...
  expect(await db.get('SELECT tenant_id,project_id,workflow_id FROM external_event_mappings WHERE id=?',destinationId)).toEqual({tenant_id:2,project_id:result.project_id,workflow_id:tenantDefault?null:result.id_map.workflows['workflow:102']});
...
it('does not activate a retired historical signal against a recreated same-key status during project copy',async()=>{
  await db.run("DELETE FROM workflow_task_statuses WHERE workflow_id=102 AND status_key='submitted_instance'");
  await db.run("INSERT INTO workflow_task_statuses(workflow_id,status_key,label,terminal) VALUES(102,'submitted_instance','New meaning',1)");
  const manifest=(await exportProjectManifest(db,101,false)).manifest;
