...
describe('markTaskNeedsAttentionForMissingSemanticHandoff', () => {
  it('records a structured operator recovery note without auto-moving visible workflow status', () => {
    ...
    const task = db.prepare(`SELECT status, previous_status FROM tasks WHERE id = 403`).get() as { status: string; previous_status: string | null } | undefined;
    expect(task).toEqual({ status: 'review', previous_status: null });

    const statusHistory = db.prepare(`SELECT * FROM task_history WHERE task_id = 403 AND field = 'status'`).all();
    expect(statusHistory).toHaveLength(0);

    const workflowEventHistory = db.prepare(`
  ...
  it('preserves runtime-success context in the operator note for outcome-less completions', () => {
    const db = new Database(':memory:');
    db.exec(`
      CREATE TABLE tasks (
        id INTEGER PRIMARY KEY,
  ...
  it('applies configured missing-outcome workflow event status actions while recording the event', () => {
    const db = new Database(':memory:');
    db.exec(`
      CREATE TABLE tasks (
        id INTEGER PRIMARY KEY,
    ...
    db.prepare(`INSERT INTO tasks (id, title, status, task_type, project_id, agent_id) VALUES (405, 'Task 405', 'review', 'backend', 86, 94)`).run();
    db.prepare(`INSERT INTO job_instances (id, task_id, runtime_ended_at) VALUES (2030, 405, '2026-05-01T22:00:00.000Z')`).run();
    db.prepare(`
    ...
    expect(changed).toBe('recorded_only');
    expect(db.prepare(`SELECT status FROM tasks WHERE id = 405`).get()).toEqual({ status: 'blocked' });
    expect(db.prepare(`SELECT old_value, new_value FROM task_history WHERE task_id = 405 AND field = 'status'`).get()).toEqual({ old_value: 'review', new_value: 'blocked' });
    expect(db.prepare(`SELECT new_value FROM task_history WHERE task_id = 405 AND field = 'workflow_event_action_kind'`).get()).toEqual({ new_value: 'status' });
    expect(db.prepare(`SELECT new_value FROM task_history WHERE task_id = 405 AND field = 'workflow_event_action_target'`).get()).toEqual({ new_value: 'blocked' });
    const note = db.prepare(`SELECT content FROM task_notes WHERE task_id = 405`).get() as { content: string };
