  ...
  const fetchImpl = async (url, init = {}) => {
    ...
    if (url.endsWith('/api/v1/setup/starter-plan/preview') && init.method === 'POST') {
      ...
      return jsonResponse(200, {
        ok: true,
        plan: {
          template: { key: body.template_key, label: 'Software: implementation + QA' },
    ...
    if (url.endsWith('/api/v1/setup/starter-plan/apply') && init.method === 'POST') {
      const body = JSON.parse(init.body);
      assert.equal(body.template_key, 'software-qa');
  ...
  await runInit(
    { apiUrl: 'http://agent-hq.test' },
    {
      io: makeIo([
        ...
        'y', 'openclaw', '', 'gw-token',
        'y', 'software-qa', 'Acme App', 'Delivery', 'Dev Owner', 'QA Owner', 'PM Owner',
